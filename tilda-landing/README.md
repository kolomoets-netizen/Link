...
| 2a | `08-kontur-search-widget.html` | Виджет поиска Контур.Закупки (iframe) + пояснение |
...
**Смысл:** отсев — «только поиск» → виджет Контур (без формы); «система для отдела» → ссылка `#demo` вверх по странице.
